...
def _solve_scipy(ham, k, eig_vectors=False, neig=0, soc=True):
    ...
    if neig:
        ...
        if eig_vectors:
            eigs, evecs = slg.eigsh(sparce_hk,
                                    neig,
                                    return_eigenvectors=True,
                                    which="SR")
            return eigs, evecs
        else:
            eigs = slg.eigsh(sparce_hk,
                             neig,
...
def solve_k_scipy(ham,
    ...
    if method == "mesh":
        ...
        if dim == 2:
            x_p = np.linspace(0, 1, nk)
            k = np.hstack((np.vstack(np.meshgrid(x_p, x_p)).reshape(2, -1).T,
                           np.zeros(nk**2).reshape(-1, 1)))
        if dim == 1:
            x_p = np.linspace(0, 1, nk)
            k = np.hstack((np.vstack(np.meshgrid(x_p)).reshape(1, -1).T,
                           np.zeros(2 * nk).reshape(-1, 2)))
    if method == "list":
        if klist != None:
            k = klist
        else:
