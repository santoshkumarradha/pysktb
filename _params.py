...
def get_hop_int(V_sss=0, V_sps=0, V_pps=0, V_ppp=0,
                V_sds=0, V_pds=0, V_pdp=0, V_dds=0, V_ddp=0, V_ddd=0,
    ...
    hop_int[1][7] = 0.5 * sqrt(3) * l * (l ** 2 - m ** 2) * V_pds + l * (1. - l ** 2 + m ** 2) * V_pdp
    hop_int[1][8] = l * (n ** 2 - 0.5 * (l ** 2 + m ** 2)) * V_pds - sqrt(3) * l * n ** 2 * V_pdp
    ...
    hop_int[2][7] = 0.5 * sqrt(3) * m * (l ** 2 - m ** 2) * V_pds - m * (1. + l ** 2 - m ** 2) * V_pdp
    hop_int[2][8] = m * (n ** 2 - 0.5 * (l ** 2 + m ** 2)) * V_pds - sqrt(3) * m * n ** 2 * V_pdp
    ...
    hop_int[3][7] = 0.5 * sqrt(3) * n * (l ** 2 - m ** 2) * V_pds - n * (l ** 2 - m ** 2) * V_pdp
    hop_int[3][8] = n * (n ** 2 - 0.5 * (l ** 2 + m ** 2)) * V_pds + sqrt(3) * n * (l ** 2 + m ** 2) * V_pdp
    hop_int[7][3] = -hop_int[3][7]
    hop_int[8][3] = -hop_int[3][8]

    hop_int[4][4] = l ** 2 * m ** 2 * (3 * V_dds - 4 * V_ddp + V_ddd) + (l ** 2 + m ** 2) * V_ddp + n ** 2 * V_ddd
    hop_int[5][5] = m ** 2 * n ** 2 * (3 * V_dds - 4 * V_ddp + V_ddd) + (m ** 2 + n ** 2) * V_ddp + l ** 2 * V_ddd
    hop_int[6][6] = n ** 2 * l ** 2 * (3 * V_dds - 4 * V_ddp + V_ddd) + (n ** 2 + l ** 2) * V_ddp + m ** 2 * V_ddd

    hop_int[4][5] = l * m ** 2 * n * (3 * V_dds - 4 * V_ddp + V_ddd) + l * n * (V_ddp - V_ddd)
    hop_int[4][6] = n * l ** 2 * m * (3 * V_dds - 4 * V_ddp + V_ddd) + n * m * (V_ddp - V_ddd)
    hop_int[5][6] = m * n ** 2 * l * (3 * V_dds - 4 * V_ddp + V_ddd) + m * l * (V_ddp - V_ddd)
    hop_int[5][4] = hop_int[4][5]
    hop_int[6][4] = hop_int[4][6]
    hop_int[6][5] = hop_int[5][6]

    hop_int[4][7] = 0.5 * l * m * (l ** 2 - m ** 2) * (3 * V_dds - 4 * V_ddp + V_ddd)
    hop_int[5][7] = 0.5 * m * n *((l ** 2 - m ** 2) * (3 * V_dds - 4 * V_ddp + V_ddd) - 2 * (V_ddp - V_ddd))
    hop_int[6][7] = 0.5 * n * l *((l ** 2 - m ** 2) * (3 * V_dds - 4 * V_ddp + V_ddd) + 2 * (V_ddp - V_ddd))

    hop_int[7][4] = hop_int[4][7]
    hop_int[7][5] = hop_int[5][7]
    hop_int[7][6] = hop_int[6][7]

    hop_int[4][8] = sqrt(3) * (l * m * (n ** 2 - 0.5 * (l ** 2 + m ** 2)) * V_dds - 2 * l * m * n ** 2 * V_ddp + 0.5 * l * m * (1. + n ** 2) * V_ddd)
    hop_int[5][8] = sqrt(3) * (m * n * (n ** 2 - 0.5 * (l ** 2 + m ** 2)) * V_dds + m * n * (l ** 2 + m ** 2 - n ** 2) * V_ddp - 0.5 * m * n * (l ** 2 + m ** 2) * V_ddd)
    hop_int[6][8] = sqrt(3) * (n * l * (n ** 2 - 0.5 * (l ** 2 + m ** 2)) * V_dds + n * l * (l ** 2 + m ** 2 - n ** 2) * V_ddp - 0.5 * n * l * (l ** 2 + m ** 2) * V_ddd)
